...
![스스로 데이터의 패턴을 학습하는 컴퓨터](./img/computer_self_learning.png)
...
쉽게 생각해서 **컴퓨터가 시험공부를 하는 다양한 방식**이라고 볼 수 있습니다.

![시험공부를 하는 컴퓨터와 지니](./img/computer_taking_exam.png)



이러한 머신러닝의 3가지 핵심 공부법에 대해서 비교해 봅니다.
...
![정답 선생님 지니와 지도 학습](./img/supervised_learning_teacher.png)
...
**예시**: 손글씨 숫자 이미지를 보고 컴퓨터가 이것이 어떤 숫자인지 맞히게 **훈련**하는 예시가 있습니다.
...
컴퓨터는 이러한 정답 데이터를 보고 자기가 **예측한 답과 비교해가며 훈련**합니다.
...
**군집화(Clustering)**는 정답(레이블)이 주어지지 않은 데이터 속에서 **서로 닮은 특징을 가진 데이터끼리 스스로 무리를 짓는(그룹화하는) 기술**입니다.

![닮은 특징끼리 스스로 묶는 군집화](./img/clustering_grouping_concept.png)

* **동작 방식**: 사람이 미리 "이것은 A, 저것은 B"라고 정답을 알려주지 않아도, 컴퓨터가 **데이터 간의 거리와 유사도**를 스스로 계산하여 **비슷한 성질**의 것끼리 같은 주머니(클러스터)로 묶어냅니다.
* **대표적인 활용**:
  - **고객 세분화**: 쇼핑몰에서 비슷한 구매 성향과 활동 패턴을 가진 고객 그룹을 자동으로 분류
  - **뉴스/문서 묶음**: 수많은 기사 중 비슷한 주제의 이야기를 하나의 그룹으로 자동 분류
  - **이상 탐지**: 일반적인 군집에서 멀리 동떨어진 비정상 데이터(이상 거래, 설비 고장 등) 발견
...
**차원 축소(Dimensionality Reduction)**는 데이터가 가진 **핵심 정보와 특징은 최대한 보존하면서, 너무 복잡하고 방대한 변수(차원)의 개수를 줄여주는 기술**입니다.

![고차원 데이터를 2차원 지도로 압축하는 차원 축소](./img/dimensionality_reduction_map.png)

* **왜 필요할까요?**: 데이터에 수백~수천 개의 특징(예: 고해상도 이미지의 수많은 픽셀)이 들어있으면 계산이 매우 무거워지고 **사람이 한눈에 이해하기 어렵습니다.** 차원 축소는 이러한 복잡한 데이터를 알맹이 정보만 남겨 가볍게 압축해 줍니다.
* **주요 목적 및 활용**:
  - **데이터 시각화**: 사람이 직접 볼 수 없는 고차원 데이터를 2차원 평면 지도(t-SNE, PCA 등)로 펼쳐서 데이터의 분포를 한눈에 확인
  - **학습 속도 향상**: 불필요하게 거대한 데이터 크기를 줄여 머신러닝 모델의 연산 속도를 대폭 개선
  - **노이즈 제거**: 중복되거나 무의미한 잡음을 걸러내어 핵심 패턴을 더 선명하게 파악
...
![에이전트와 환경의 상호작용 및 보상](./img/rl_interaction_concept.png)
...
에이전트(기계/배우는 주체)가 자신이 놓인 **환경**과 실시간으로 주거니 받거니 **상호작용**하며 자가 학습을 수행합니다.
...
> 강화학습의 5가지 요소에 대해서는 나중에 다시한번 상세히 다루어 보도록 합니다.
...
![누적 보상의 최대화를 목표로 하는 강화 학습](./img/rl_max_reward.png)
...
로봇에게 걷는 방법을 가르쳐줄 완벽한 인간 선생님을 구하기란 매우 어렵습니다. 

(각 관절의 모터 각도를 0.01초 단위로 완벽하게 텍스트로 적어줄 정답 해설지가 존재하지 않기 때문입니다.)
...
로봇은 일단 임의로 다리를 구부려 봅니다. 

넘어지면 마이너스 보상을 받고, 1cm라도 앞으로 전진하면 플러스 보상(전진 거리)을 얻습니다. 

![수많은 시행착오를 거쳐 걷는 법을 깨우치는 로봇](./img/robot_walking_trial_error.png)

로봇은 이 무수한 **시행착오** 경험 데이터를 스스로 수집하고 업데이트해가며 자전거 균형을 잡듯 걷는 행동 패턴을 자연스럽게 익힙니다.
...
그럼 기존의 학습방법과 강화학습 간에는 어떠한 차이점들이 있을까요?
...
![머신러닝 3대 공부법 핵심 요약](./img/ml_summary_ch3_1.png)
