...
**그림 05-1-3** 그리드 월드(Grid World)의 탐험 시작: 상태(격자 위치), 행동(이동 방향), 보상(사과와 보물)으로 구성된 강화학습의 기초 실험 환경
...
**그림 05-1-4** 그리드 월드에서의 상태 정의와 보상 요소
...
**그림 05-1-5** 타임 스텝 <i>t</i>의 흐름에 따른 상태 전이와 보상 획득 과정 (왼쪽 이동 경로 vs 오른쪽 이동 경로)
...
**그림 05-1-6** 단기 보상 vs 장기 누적 보상의 딜레마: 눈앞의 작은 사과(+1) 대신 함정(-2) 너머의 황금 사과 바구니(+6)를 얻어 총합(+4)을 극대화하는 최선의 선택
...
**그림 05-1-7** 에이전트와 환경의 상호작용 비교: 외부 환경의 피드백이 없는 개방 루프(Open-Loop) vs 상태와 보상을 실시간으로 주고받는 동적 폐루프(Closed-Loop)
...
**그림 05-1-8** MDP 에이전트와 환경의 3단계 상호작용 폐루프 (Observation &rarr; Action &rarr; Reward & Next State)
...
**그림 05-1-9** 시간의 흐름(<i>t</i> = 0, 1, 2, ..., <i>T</i>)에 따른 상호작용 시계열 데이터와 상태-행동-보상 궤적(Trajectory)
...
**그림 05-1-10** 보상 표기법 비교: 행동 시점 기준(<i>R<sub>t</sub></i>) vs 도착 상태 시점 기준(<i>R<sub>t+1</sub></i>)
...
**그림 05-1-11** 05.1 마르코프 결정 과정(MDP) 핵심 개념 요약 인포그래픽
