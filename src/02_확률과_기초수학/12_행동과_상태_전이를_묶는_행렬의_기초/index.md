...
* **행렬(Matrix)**: 수나 문자를 직사각형 모양으로 배열하여 대괄호(`[ ]`)로 묶은 표 형태의 수학적 상자입니다.
  - **행(Row, 가로줄)**: 행렬의 가로 방향 줄
  - **열(Column, 세로줄)**: 행렬의 세로 방향 줄
  - *M*개의 행과 *N*개의 열을 가진 행렬을 **_M × N_ 행렬**이라 부릅니다.
* **벡터(Vector)**: 단 한 줄(가로 또는 세로)로만 이루어진 특수한 형태의 1차원 행렬입니다.
  - **행 벡터(Row Vector)**: 가로로 1줄만 있는 1 × *N* 크기의 벡터 (예: 강화학습의 현재 상태 확률 분포)
  - **열 벡터(Column Vector)**: 세로로 1줄만 있는 *M* × 1 크기의 벡터

![행렬과 벡터의 구조](./img/dorothy_matrix_vector_structure.png)
**그림 02-12-2** 가로 행(Row)과 세로 열(Column)로 구성된 행렬(Matrix)과 1줄짜리 행 벡터(1 × N), 열 벡터(M × 1)의 구조를 설명하는 지니와 도로시, 토토

"아하! 가로 방향이 '행(Row)'이고 세로 방향이 '열(Column)'이구나! 현재 상태 확률 분포는 가로로 긴 '행 벡터'로 나타내면 딱 맞네!"

도로시가 바둑판 칸을 짚으며 즐겁게 말했습니다.
...
**그림 02-12-3** 행 벡터의 가로 행 성분과 전이 행렬의 세로 열 성분을 차례대로 짝지어 곱한 뒤 모두 더해주는 행렬-벡터 곱셈의 연산 메커니즘
...
* **곱셈의 성립 조건**: 앞 행렬의 **열 개수**와 뒤 행렬의 **행 개수**가 반드시 일치해야 합니다. (1 × 2 벡터 × 2 × 2 행렬 ➔ 1 × 2 결과 벡터)
...
**그림 02-12-4** 오늘의 확률분포 벡터(**x**₀)와 전이 확률 행렬(**P**)을 단 한 번 곱하여 내일의 날씨 확률분포(맑음 58%, 비 42%)를 도출하는 마르코프 체인 연산

---

### 2.12.3.2 1단계 및 다단계 상태 전이 계산

내일 날씨의 확률분포 **x**₁은 행렬 곱셈 한 번으로 가뿐히 계산됩니다.
...
$$
* **내일 날씨 벡터**: **x**₁ = [0.58, 0.42]

![다단계 상태 전이와 행렬 거듭제곱](./img/dorothy_markov_multistep_transition.png)
**그림 02-12-5** 내일(**x**₁ = **x**₀**P**), 모레(**x**₂ = **x**₀**P**²), 10일 후(**x**₁₀ = **x**₀**P**¹⁰)의 미래 확률분포를 행렬 거듭제곱으로 단숨에 계산하고 장기 정상 상태로 수렴하는 원리를 보여주는 지니와 도로시, 토토

"와! 모레 날씨 **x**₂를 구하고 싶으면 **x**₁**P** = **x**₀**P**²로 행렬만 거듭제곱하면 되네! 수많은 단계의 미래 상태도 행렬의 거듭제곱으로 단숨에 풀리는구나!"
...
* **예제 파일**: [markov_transition_matrix.py](markov_transition_matrix.py)
...
> **💡 코드 해석**: 날이 지날수록 날씨 확률 분포는 초기 조건(맑음 60%)과 상관없이 시스템 고유의 고정점인 **정상 상태 분포(≈ [0.5714, 0.4286])**로 수렴합니다. 이는 강화학습에서 정책(Policy)이 수렴할 때의 장기 정상 상태 분석에 핵심적으로 활용됩니다.
...
> 현재 에이전트의 위치 확률분포 벡터 **x** = [0.5, 0.5] 와 이동 전이 행렬 **P** = [[0.8, 0.2], [0.3, 0.7]] 의 행렬 곱 **xP**를 계산하시오:
$$
\mathbf{x} \mathbf{P} = \begin{bmatrix} 0.5 & 0.5 \end{bmatrix} \begin{bmatrix} 0.8 & 0.2 \\ 0.3 & 0.7 \end{bmatrix}
$$
>
> **풀이**:
...
> 문제 1의 시스템에서 2단계 후의 상태 확률분포 벡터 **x**₂를 구하시오.
>
> **풀이**:
> 1단계 결과 벡터 **x**₁ = [0.55, 0.45] 에 전이 행렬 **P**를 한 번 더 곱합니다:
> 1. 1번째 원소:
$$
...
**그림 02-12-6** 행렬의 가로 행과 세로 열 구조, 가로×세로 행렬 곱셈 연산법, 마르코프 체인의 상태 전이 방정식 **x**₁ = **x**₀**P**를 완벽히 마스터한 도로시와 지니, 토토
