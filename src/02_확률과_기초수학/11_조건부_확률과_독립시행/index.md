...
도로시가 나무 지팡이를 짚은 채 두려움과 호기심이 섞인 눈빛으로 귀를 기울였습니다. 토토도 귀를 쫑긋 세우고 바스락거리는 수풀을 경계했습니다.
...
* **수식의 의미**:
  1. 분모 $P(A)$: 조건 사건 *A*가 발생하여 새로운 전체 표본공간의 기준이 됩니다.
  2. 분자 $P(A \cap B)$: 조건 *A*를 만족하는 영역 안에서 사건 *B*도 동시에 일어나는 교집합 영역의 크기입니다.

"아! '소리가 났다'는 영역으로 전체 공간이 좁혀지니까, 그 소리 구역 안에서 진짜 몬스터가 숨어 있는 교집합 비율만 계산하면 되는구나!"
...
한 사건 *A*가 일어났다는 사실이 다른 사건 *B*가 일어날 확률에 아무런 영향을 주지 않는 관계를 뜻합니다.

$$
P(B|A) = P(B) \iff P(A \cap B) = P(A) \times P(B)
$$

* **대표 예시 (복원추출)**: 주머니에서 공을 하나 꺼내 색깔을 확인한 뒤 **다시 집어넣고** 두 번째 공을 뽑을 때, 첫 번째 시행과 두 번째 시행은 완벽히 독립입니다.

---

### 2.11.2.2 종속사건 (Dependent Events)

한 사건의 발생 여부가 다른 사건의 확률에 직접적인 영향을 미치는 관계입니다.

* **대표 예시 (비복원추출)**: 주머니에서 공을 꺼낸 뒤 **다시 넣지 않고** 연이어 두 번째 공을 뽑을 때, 남은 공의 개수와 색상 비율이 변하므로 두 번째 사건은 첫 번째 사건에 종속됩니다.
...
**그림 02-11-3** 매회 사격 성공률(80%)이 일정한 독립시행 환경에서 5번의 시도 중 2번 성공할 조합과 확률을 계산하는 도로시와 지니, 토토
...
**그림 02-11-4** 몬티 홀 퀴즈 쇼 무대에서 사회자가 2번 문의 염소(꽝)를 공개했을 때, 선택을 3번 문으로 바꾸는 것이 승리 확률을 2/3(67%)로 2배 높여줌을 보여주는 시각화
...
한 반에 단 **23명의 학생**만 모여 있어도, 1년 365일 중 **생일이 같은 학생이 적어도 한 쌍 이상 존재할 확률은 약 50.7%**로 절반을 넘습니다. (30명이 모이면 무려 70.6%에 도달!)

* **여사건 증명 (모두 생일이 다를 확률)**:
$$
P(\text{모두 다름}) = \frac{365}{365} \times \frac{364}{365} \times \frac{363}{365} \times \dots \times \frac{365 - 23 + 1}{365} \approx 0.4927
$$
...
**그림 02-11-5** 표본공간의 축소를 수반하는 조건부 확률과 독립시행 공식, 몬티 홀 딜레마와 생일 패러독스의 원리를 정복한 도로시와 지니, 토토

1. **조건부 확률($P(B|A)$)**: 새로운 단서나 조건 *A*가 관측되면 전체 표본공간이 사건 *A*의 영역으로 축소되며, 그 안에서 교집합의 비율($\frac{P(A \cap B)}{P(A)}$)을 구한다.
2. **독립과 종속**: 이전 사건이 다음 사건에 영향을 미치지 않으면 **독립사건($P(A \cap B) = P(A)P(B)$)**, 영향을 미치면 **종속사건**이다.
3. **독립시행의 정리**: 매회 성공률 *p*가 일정한 독립 실험을 *n*번 반복하여 *r*번 성공할 확률은 **$_nC_r p^r (1-p)^{n-r}$**로 계산한다.
